...
class Situation:  # 环境
    def __init__(self, J_num, M_num, O_num, J, Processing_time, D, Ai):  #
        ...
        self.CTI = [0 for i in range(J_num)]  # 各工件上最后一道工序的完工时间列表
    ...
    def Features(self):
        ...
        # 4 平均工件工序完成率
        CRJ_ave = sum(self.CRJ) / self.J_num  # 平均工件工序完成率，# 初始为0
        ...
        # T_cur = sum(self.CTK) / self.M_num  # 每台机器最后一道工序结束的平均时间点
        N_tard, N_left = 0, 0  # 估计延迟的工序数，剩余未加工的工序数
        for i in range(self.J_num):  # 对每个工件计算
            if J[i] > self.OP[i]:  # 总工序数大于已加工工序数
                N_left += self.J[i] - self.OP[i]  # 剩余工序数
                T_left = 0  # 剩余时间，计算单个工件的剩余加工时间
                for j in range(self.OP[i] + 1, J[i]):  # 剩余工序遍历索引
                    M_ij = [k for k in self.Processing_time[i][j] if k > 0 and k < 999]  # 剩余工序加工时间集合
                    T_left += sum(M_ij) / len(M_ij)  # 剩余工序在可用机器的平均加工时间之和
                    # if T_left + T_cur > self.D[i]:  # 剩余平均加工时间之和与当前平均结束时间之和大于截止时间
                    if T_left + self.CTI[i] > self.D[i]:  # 剩余平均加工时间之和与当前平均结束时间之和大于截止时间
                        N_tard += self.J[i] - j + 1  # 延期工序累积数量
                        break
        try:
            Tard_e = N_tard / N_left  # 平均延期程度 # 初始为0，所有工序的延期程度
        except:
            Tard_e = 0
        ...
        for i in range(self.J_num):
            if J[i] > self.OP[i]:
                ...
                try:
                    if self.CTI[i] > self.D[i]:  # 工序self.OP[i]的结束时间大于截止时间
                        N_tard += self.J[i] - self.OP[i]  # 延期数量
                except:
                    pass
        try:
    ...
    def rule1(self):
        ...
        for i in range(len(self.CTK)):  # 机器数遍历
            if self.Processing_time[Job_i][On][i] != -1:  # 工件最后一道工序在i机器的结束时间不等于-1
    ...
    def scheduling(self, action):  # 根据行动（选择的工件，机器），将加工工序所在工件、加工位置、加工时间开始、结束均存储起来
        ...
        PT = self.Processing_time[Job][O_n][Machine]  # 即将加工的工序加工时间
    ...
    def reward(self, Ta_t, Te_t, Ta_t1, Te_t1, U_t, U_t1):
       ...
       :param U_t: U_ave(t) 设备利用率
       :param U_t1: U_ave(t+1)
       :return: reward
        '''
        if Ta_t1 < Ta_t:  # 下个状态的实际延迟率比此状态的延迟率小？？
            rt = 1
        else:
            if Ta_t1 > Ta_t:
                rt = -1
