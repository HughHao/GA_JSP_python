...
TC = 0  # 所有工序加工总时长
JT = np.zeros(J_num)  # 每个工序加工总时长
for i in range(J_num):
    for j in range(Op_num[i]):
        Mij = [t for t in Processing_time[i][j] if t != -1]
        JT[i] += sum(Mij)/len(Mij)
TC = sum(JT)
class Situation:  # 环境
    ...
    def Features(self, ite):
        ...
        CRO_ave = Ct/self.TC  # 已加工工序平均数， # 初始为0
        ...
        return U_ave, U_std, CRO_ave, CRJ_ave, CRJ_std, TARD, Tard_a, ite
    ...
    def rule1(self):
        ...
        Tard_Job = [i for i in range(self.J_num) if self.OP[i] < self.J[i] and self.D[i] < self.CTI[i]]  # 当前延期的工件索引列表
        UC_Job = [j for j in range(self.J_num) if self.OP[j] < self.J[j]]  # 还没有完工的工件索引列表
        if Tard_Job == []:  # 如果延期工件集合为空
            Job_i = UC_Job[np.argmin([(self.D[i] - self.CTI[i]) / (self.J[i] - self.OP[i]) for i in UC_Job])]  # 剩余完工时间最小工件索引
        else:
            T_ijave = []  # 平均延期工件
            for i in Tard_Job:  # 对每个延期工件,不再对另外未完成工件遍历
                Tad = []
                ...
                T_ijave.append(self.CTI[i] + sum(Tad) - self.D[i])  # 每个工件的延期程度，剩余平均加工时间与平均结束时间之和减去截止时间
            Job_i = Tard_Job[np.argmax(T_ijave)]  # 预计延期最大的工件
        '''选出工件，接下来选取机器'''
        try:  # 若此句出现问题
            C_ij = self.CTI[Job_i]  # 找出选出工件的结束时间（各工序的最大结束时间）
        except:  # 执行下句
            C_ij = self.Ai[Job_i]  # 工件i的arrival time的最大值

        A_ij = self.Ai[Job_i]  # 工件i的arrival time
        # print(A_ij)
        On = self.OP[Job_i]  # 工序数，已加工工序数含有结束时间工序的个数
        Mk = []
        for k in range(self.M_num):  # 机器数遍历
            if self.Processing_time[Job_i][On][k] != -1:  # 工件最后一道工序在i机器的结束时间不等于-1
                C_ij += self.Processing_time[Job_i][On][k]  # 该工序每次结束时间计算，直至最后一道工序
                Mk.append(max(C_ij, A_ij, self.CTK[k]))  # 可用机器k的最早开始时间
            else:
                Mk.append(9999)
    ...
    def rule2(self):
        ...
        T_cur = sum(self.CTK) / self.M_num
        # Tard_Job:不能按期完成的工件
        Tard_Job = [i for i in range(self.J_num) if self.OP[i] < self.J[i] and self.D[i] < self.CTI[i]]  # 预计有延期的工件
        UC_Job = [j for j in range(self.J_num) if self.OP[j] < self.J[j]]  # 还有工序未完成的工件索引
        T_ijave = []  #
        for i in range(self.J_num):
            Tad = []
            for j in range(self.OP[i], self.J[i]):  # 剩余工序遍历
                T_ijk = [k for k in self.Processing_time[i][j] if k != -1]  # 该工序的加工时
                Tad.append(sum(T_ijk) / len(T_ijk))  # 剩余每个工序的平均加工时间
            T_ijave.append(sum(Tad))  # 未完工工件的剩余工序平均加工时间之和，可存在空列表
        if Tard_Job == []:  # 没有延期工件
            Job_i = UC_Job[np.argmin([(self.D[i] - self.CTI[i]) / T_ijave[i] for i in UC_Job])]
        else:
            Job_i = Tard_Job[np.argmax([self.CTI[i] + T_ijave[i] - self.D[i] for i in Tard_Job])]
        try:
            C_ij = self.CTI[Job_i]
        except:
        ...
        for k in range(self.M_num):
            if self.Processing_time[Job_i][On][k] != -1:
                C_ij += self.Processing_time[Job_i][On][k]
                Mk.append(max(C_ij, A_ij, self.CTK[k]))  # 机器可以开始时间
            else:
                Mk.append(9999)
    ...
    def rule3(self):
        ...
        for i in UC_Job:
            ...
            T_ijave.append(self.CTI[i] + sum(Tad) - self.D[i])  # 延期程度
        ...
        if random.random() < 0.5:
            ...
            Machine = np.argmin(MT)  # 加工时间最短机器，负载最小
    ...
    def rule4(self):
        UC_Job = [j for j in range(self.J_num) if self.OP[j] < self.J[j]]
        Job_i = random.choice(UC_Job)  # 随即从未完成加工的工件选出一个
        try:
            C_ij = self.CTI[Job_i]
        except:
            C_ij = self.Ai[Job_i]  # 工件i的arrival time
    ...
    def rule5(self):
        ...
        T_cur = sum(self.CTK) / self.M_num
        # Tard_Job:不能按期完成的工件
        Tard_Job = [i for i in range(self.J_num) if self.OP[i] < self.J[i] and self.D[i] < self.CTI[i]]
        UC_Job = [j for j in range(self.J_num) if self.OP[j] < self.J[j]]
        if Tard_Job == []:
            Job_i = UC_Job[np.argmin([self.CRJ[i] * (self.CTI[i]-self.D[i]) for i in UC_Job])]
        else:
            T_ijave = []
            for i in Tard_Job:
                Tad = []
                for j in range(self.OP[i], self.J[i]):
                    T_ijk = [k for k in self.Processing_time[i][j] if k != -1]
                    Tad.append(sum(T_ijk) / len(T_ijk))
                T_ijave.append(1 / (self.CRJ[i] + 1) * (self.CTI[i] + sum(Tad) - self.D[i]))
    ...
    def rule6(self):
        ...
        for i in UC_Job:
            ...
            for j in range(self.OP[i], self.J[i]):
                T_ijk = [k for k in self.Processing_time[i][j] if k != -1]
                Tad.append(sum(T_ijk) / len(T_ijk))
            T_ijave.append(self.CTI[i] + sum(Tad) - self.D[i])
        Job_i = UC_Job[np.argmax(T_ijave)]
        try:
            C_ij = self.CTI[Job_i]
        except:
    ...
    def scheduling(self, action):  # 根据行动（选择的工件，机器），将加工工序所在工件、加工位置、加工时间开始、结束均存储起来
        ...
        PT = self.Processing_time[Job][O_n][Machine]  # 即将加工(目前工序号+1)的工序加工时间
    ...
    def reward(self, obs, obs_t, episode):
        #  obs[0-6]:设备平均利用率，设备利用率标准差、所有工序时间完成率、每个工件工序完成率平均值、平均工件工序完成率标准差、预估延迟程度(下一状态更好-小)、当前已延迟数量比例
        # print([Ta_t, Te_t, Ta_t1, Te_t1, U_t, U_t1])
        # rt = np.exp(Te_t)/np.exp(Te_t1) + np.exp(Ta_t)/np.exp(Ta_t1) + np.exp(U_t1)/np.exp(U_t)
        # r1 = (obs_t[0]-obs[0])*1/(1 + np.exp(-1 * episode))  # 平均设备利用率的奖励

        # r1 = (obs_t[0]-obs[0])*(math.log(episode+1, np.exp(1)))*(1 + np.exp(-1 * (obs_t[0]-obs[0])))*1/(1 + np.exp(-1 * episode))
        r1 = (obs_t[0] - obs[0]) * (1 + np.exp(-1 * (obs_t[0] - obs[0]))) * 1 / (1 + np.exp(-1 * episode))
        r2 = 0
        for i in range(len(self.UK)):
            r2 += 1/(1+np.exp(self.UK[i]))
