...
def update_AK(A0, r0, K, t, c_r0, mu0):  # 更新参数
    ...
    mu = mu0*(1-A/K)
    # pop_size = 10
    pop_size = math.ceil(math.log(K / A0)) + 4  # 种群规模
    ...
    for k in range(pop_size):
        ...
        if pk > max(select_r, A/K):
            ...
            if fk < fb_global:
                ...
                Xk, X = two_points_cross(X_, Xb)
            else:
                random.shuffle(Xk)
            # Xk = copy.deepcopy(Xb)
        # random.shuffle(Xk)
        else:
            if mu0 > random.random():
                target = random.choice(target_points)  # ???三种编译策略
                # target = 3
                if target == 1:
                    Xk = gene_exchange(Xk)
                elif target == 2:
                    Xk = gene_insertion(Xk)
                else:
                    Xk = gene_reverse(Xk)
            if c_r0 > random.random():
                Xk, X = two_points_cross(Xk, Xb)
        # else:
        #     Xk = copy.deepcopy(Xb)
        #     random.shuffle(Xk)
        fk = node(Xk).makespan
...
print(node(Xb_global).load_table)
config = {
            "font.family": 'serif',
            "font.size": 20,
